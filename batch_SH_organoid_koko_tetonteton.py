...
if "_C5_CystMask.tiff" in name_image:
    #print(nm)
    df = pd.DataFrame(columns = col_names)
    #df
    fileName = name_image.replace('_C5_CystMask.tiff','')
    ...
    mask = imread(os.path.join(inputDir, str(fileName + "_C5_CystMask.tiff"))) # mask segmeted cysts
    C1 = imread(os.path.join(inputDir, str(fileName + "_C1.tif"))) # FoxA2 staining
    C2 = imread(os.path.join(inputDir, str(fileName + "_C2.tif"))) # FoxA2 genotype stainig
    C3 = imread(os.path.join(inputDir, str(fileName + "_C3.tif"))) # Pax6 genotype stainig
    C4 = C2 + C3
